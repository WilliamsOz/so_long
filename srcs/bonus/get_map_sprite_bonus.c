...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   get_map_sprite_bonus.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: wiozsert <wiozsert@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/03 17:09:08 by wiozsert          #+#    #+#             */
/*   Updated: 2021/11/05 16:29:19 by wiozsert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_wall_sprite_bonus(t_mlx mlx, int x, int y)
{
	...
	while (mlx.lin < 32)
	{
		...
		while (mlx.col < 32)
		{
			mlx.buffer = (unsigned int)mlx.sprite_addr + (y * mlx.line_lenght
...
{
	mlx.sprite_img = mlx_xpm_file_to_image(mlx.mlx_id,
			"./textures/monster.xpm", &mlx.width, &mlx.height);
	mlx.sprite_addr = mlx_get_data_addr(mlx.sprite_img, &mlx.bits_per_pixel,
			&mlx.line_lenght, &mlx.endian);
	while (mlx.lin < 32)
	{
	...
	while (x < mlx.x_max)
	{
		while (map[x][y] != '\0')
		{
			...
			if (map[x][y] == 'P')
				...
				get_wall_sprite_bonus(mlx, x * 32, y * 32);
			else if (map[x][y] == '0')
                ...
                get_monster_sprite(mlx, x * 32, y * 32);
			y++;
		}
		y = 0;
		x++;
