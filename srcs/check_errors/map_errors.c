...
/*   Updated: 2021/11/03 17:35:29 by wiozsert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	map_errors(t_maperr e, int ind)
{
	if (ind == 1 && (e.malloc_failed == 1 || e.empty_map == 1))
	{
		...
		return (1);
			...
			|| e.too_many_player == 1 || e.empty_line == 1
			|| e.is_map_open == 1 || e.map_is_square == 1
			|| e.map_too_short == 1 || e.map_is_not_rectangle == 1))
	{
		write(1, "Error\n", 6);
