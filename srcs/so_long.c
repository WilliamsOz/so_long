...
/*   Updated: 2021/11/05 19:28:24 by wiozsert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_starter(char **map, t_maperr e)
{
	...
	get_map_sprite(mlx, 0, 0, map);
	mlx_key_hook(mlx.win_id, key_hook, &mlx);
...
int	main(int ac, char **av)
{
	...
	else if (check_errors(&map, av, &error) == 1)
		return (0);
	else
...
Pressing ESC must close the window and quit the
program cleanly.


BONUS PART :

enemy patrols that cause the player to lose in
case of contact.

There’s some sprite animation.

Movement count is directly displayed on the
screen instead of shell output.
